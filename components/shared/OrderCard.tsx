...
export default function OrderCard({ order, onClick }: { order: SalesOrder; onClick?: () => void }) {
  ...
  return (
    <div
      onClick={onClick}
      className={cn(
        "rounded-card border border-rsl-border bg-surface p-3.5 shadow-card",
        onClick && "cursor-pointer hover:shadow-pop transition-shadow"
      )}
    >
      <div className="flex items-center justify-between mb-1.5">
        <span className="text-body-md font-bold text-ink">{order.orderNumber}</span>
      ...
      {order.seller && <p className="text-meta font-bold text-ink">{fullName(order.seller)}</p>}
